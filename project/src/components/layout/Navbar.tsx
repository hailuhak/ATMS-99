...
export const Navbar: React.FC = () => {
  ...
  // Resize, convert to Base64, and upload profile image to Firestore
 const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  try {
    ...
    while (base64String.length > 950000 && quality > 0.1) { // slightly under 1 MB
      quality -= 0.05;
  ...
  return (
    <>
      <motion.nav className="fixed top-0 left-0 w-full z-50 bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700" initial={{ y: -100 }} animate={{ y: 0 }} transition={{ duration: 0.3 }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            {/* Logo */}
            <motion.h1 className="text-2xl font-bold text-blue-600 dark:text-blue-400" whileHover={{ scale: 1.05 }}>ATMS</motion.h1>

            {/* Desktop Menu */}
            <div className="hidden lg:flex items-center space-x-4">
              <Button variant="ghost" size="sm" onClick={() => setSettingsOpen(!settingsOpen)}><Settings className="w-4 h-4" /></Button>
              <Button variant="ghost" size="sm">
                <div className="relative">
                  <Bell className="w-5 h-5" />
                  {pendingUsers > 0 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs w-4 h-4 flex items-center justify-center rounded-full">{pendingUsers}</span>}
                  {gradeCount > 0 && <span className="absolute -bottom-1 -right-1 bg-green-500 text-white text-xs w-4 h-4 flex items-center justify-center rounded-full">{gradeCount}</span>}
                </div>
              </Button>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div className="relative">
                  <label htmlFor="profile-upload" className="cursor-pointer">
                    {profileImage ? <img src={profileImage} alt="Profile" className="w-8 h-8 rounded-full object-cover border-2 border-blue-500" /> :
                  ...
                  {uploading && <div className="absolute inset-0 bg-black/40 flex items-center justify-center rounded-full"><span className="text-xs text-white">...</span></div>}
                </div>
              </div>
            ...
            <div className="lg:hidden flex items-center space-x-2">
              <Button variant="ghost" size="sm" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}><Menu className="w-5 h-5" /></Button>
              <div className="relative">
        ...
        {settingsOpen && (
          <div className="absolute right-4 top-16 w-64 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 shadow-lg rounded-lg p-4 z-50">
            ...
            {loadingInfo ? <p className="text-gray-500 dark:text-gray-400">Loading...</p> : (
              <>
                <div className="mb-2">
                  <label className="block text-sm text-gray-700 dark:text-gray-300">Display Name</label>
                  <input type="text" value={userInfo.displayName} onChange={e => setUserInfo({ ...userInfo, displayName: e.target.value })} className="w-full p-1 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-200" />
                </div>
                <div className="mb-2">
                  <label className="block text-sm text-gray-700 dark:text-gray-300">Email</label>
                  <input type="email" value={userInfo.email} onChange={e => setUserInfo({ ...userInfo, email: e.target.value })} className="w-full p-1 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-200" />
                </div>
                <Button variant="primary" size="sm" onClick={handleUpdateInfo} className="w-full mt-2">Update Info</Button>
              </>
            )}
          </div>
        )}
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 shadow-md">
            <div className="flex flex-col p-4 space-y-2">
              ...
              <Button variant="ghost" size="sm" onClick={toggleTheme}>{theme === 'light' ? <Moon className="w-4 h-4 mr-2" /> : <Sun className="w-4 h-4 mr-2" />} Theme</Button>
            </div>
          </div>
        )}
      </motion.nav>
