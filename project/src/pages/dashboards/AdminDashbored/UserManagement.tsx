...
//     try {
//       // Normal admin cannot add another admin
//       if (role === "admin" && !currentUserData?.isSuperAdmin) {
//         alert("Only super-admin can add another admin!");
//         return;
//       }

//       // Check for existing user
//       const userRef = collection(db, "users");
//       const existingUserQuery = query(userRef, where("email", "==", email));
...
//       const currentUser = auth.currentUser;
//       if (currentUser && currentUser.uid === editingUser.uid && currentUser.email !== editingUser.email) {
//         await updateEmail(currentUser, editingUser.email);
//       }
...
//                   | "trainer"
//                   | "admin";
//                 if (selectedRole === "admin" && !currentUserData?.isSuperAdmin) {
//                   alert("Only super-admin can add another admin!");
//                   return;
//                 }
...
//               <option value="trainee">Trainee</option>
//               <option value="trainer">Trainer</option>
//               {currentUserData?.isSuperAdmin && <option value="admin">Admin</option>}
//             </select>
//             <div className="flex gap-2">
//               <Button onClick={handleAddUser}>Save User</Button>
//               <Button variant="outline" onClick={() => setShowAddUserForm(false)}>
...
//                 <tbody>
//                   {users.map((user) => (
//                     <tr key={user.id || user.uid} className="border-b dark:border-gray-600">
//                       <td className="px-4 py-2">{user.displayName || "N/A"}</td>
//                       <td className="px-4 py-2">{user.email}</td>
//                       <td className="px-4 py-2 capitalize">{user.role}</td>
//                       <td className="px-4 py-2">
...
//                         ) : (
//                           <>
//                             {/* Disabled icons for normal admin */}
//                             <Edit2 className="w-5 h-5 text-gray-400 cursor-not-allowed" />
